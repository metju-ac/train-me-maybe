...
func FetchRouteDetail(apiClient *openapiclient.APIClient, input *models.UserInput) (*openapiclient.Route, error) {
	...
	routeDetail, httpRes, err := apiClient.RoutesAPI.GetSimpleRouteDetail(ctx, input.SelectedRouteIds).FromStationId(input.DepartingStation.StationID).ToStationId(input.ArrivingStation.StationID).Execute()
	if err != nil {
