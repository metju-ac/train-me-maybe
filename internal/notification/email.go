...
func formatConnectionShort(input *models.UserInput) string {
	return fmt.Sprintf("%s -> %s on %s",
		input.DepartingStation.City,
		input.ArrivingStation.City,
		input.RouteDetail.DepartureTime.Format("02.01. 15:04"))
}

func formatRoute(route *openapiclient.Route) string {
	return fmt.Sprintf("%s - %s (Transfers: %d)",
		route.DepartureTime.Format("02.01. 15:04"),
		route.ArrivalTime.Format("02.01. 15:04"),
		len(route.GetTransfersInfo().Transfers))
}
...
// EmailNotificationFreeSeats partly taken from https://www.loginradius.com/blog/engineering/sending-emails-with-golang/
func EmailNotificationFreeSeats(config *config.SMTPConfig, input *models.UserInput) {
	slog.Info("Preparing free seats email notification",
		"departingStation", input.DepartingStation.StationID,
		"arrivingStation", input.ArrivingStation.StationID,
		"selectedRoutes", input.SelectedRouteIDs)

	subject := `[TRAIN ME MAYBE] Free seats found on ` + formatConnectionShort(input)
	body := `There are free seats on the route from <b>` +
		cli.FormatStation(input.DepartingStation) + `</b> to <b>` +
		cli.FormatStation(input.ArrivingStation) + `</b> on the selected route: ` +
		formatRoute(input.RouteDetail)
...
func generateTicketBoughtEmailBody(input *models.UserInput, ticket *openapiclient.Ticket, beersOwed *float32) string {
	...
	if beersOwed != nil {
		body += `<br><br> You now owe us ` + fmt.Sprintf("%.2f", *beersOwed) +
			` beers for our amazing service. Thanks for keeping Robert and Matěj hydrated 🍻.`
	}
	return body
}

func EmailNotificationTicketBought(config *config.SMTPConfig, input *models.UserInput, ticket *openapiclient.Ticket) {
	slog.Info("Preparing bought ticket email notification",
		"departingStation", input.DepartingStation.StationID,
		"arrivingStation", input.ArrivingStation.StationID,
		"selectedRoutes", input.SelectedRouteIDs)
...
func EmailNotificationTicketBoughtWithBeers(
	config *config.SMTPConfig,
	input *models.UserInput,
	ticket *openapiclient.Ticket,
	beersOwed float32,
) {
	slog.Info("Preparing bought ticket email notification with beers owed",
		"departingStation", input.DepartingStation.StationID,
		"arrivingStation", input.ArrivingStation.StationID,
		"selectedRoutes", input.SelectedRouteIDs)
...
func EmailNotificationTicketNotPaid(config *config.SMTPConfig, input *models.UserInput, price float32, currency string) {
	...
	body := `Your ticket purchase has failed. The ticket for <b>` + fmt.Sprintf("%.2f", price) +
		` ` + currency + `</b> has been booked but not been paid. Please, check that you have enough credit. ` +
		`If you do, please report this issue to the support.`
	sendEmail(config, input, subject, body)
}
